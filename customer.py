...
class Tester:
    ...
    def encode(self, code , msg):
        ...
        self.encoded_msg = '0' * (10 - (len(bin(code)))) + bin(code).replace("0b", '') + '0' * (32 - len(bin(len(self.encoded_msg)).replace('0b', ''))) + bin(len(self.encoded_msg)).replace('0b', '') + self.encoded_msg
    ...
    def send_data(self):
        self.sock.sendall(self.encoded_msg.encode())
